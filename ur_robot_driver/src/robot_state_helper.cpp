...
bool RobotStateHelper::safeDashboardTrigger(rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr srv)
{
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("robot_state_helper"), "Service response received: " << response->message);

  if (!response->success) {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("robot_state_helper"),
                        "Service call failed with message: " << response->message);
    return false;
  }
  return true;
}
